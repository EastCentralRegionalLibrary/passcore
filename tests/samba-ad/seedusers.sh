...
samba-tool ou create "${ou_people}"
samba-tool ou create "${ou_groups}"
...
samba-tool user create admin adminpassword \
  --userou="${ou_people}" --use-username-as-cn \
...
samba-tool group add AllowedGroup --groupou="${ou_groups}"
samba-tool group add RestrictedGroup --groupou="${ou_groups}"
...
samba-tool group add NestedOuter --groupou="${ou_groups}"
samba-tool group add NestedInner --groupou="${ou_groups}"
...
if samba-tool group add --help 2>&1 | grep -q -- "--group-type"; then
  samba-tool group add DistributionGroup --groupou="${ou_groups}" --group-type=Distribution
else

...
- To allow an administrative reset when the normal password change is rejected
  - Find the `AllowAdministrativeReset` entry (in `AppSettings`, applies to both the AD and LDAP providers, default `false`). When `false`, a change rejected by the domain — password history, minimum password age, or a cannot-change-password restriction — is reported to the user as a policy error. When `true`, PassCore retries the change as an administrative reset using the configured service account, which **bypasses password history and minimum-age policy**. The reset can only ever fire after the user's current password was verified in the same request, never for wrong-password or connectivity failures, and every occurrence is logged at Warning level with the request's correlation ID. It is ignored when the AD provider runs with `UseAutomaticContext: true` (no service account to reset with) or when the LDAP provider uses `LdapChangePasswordWithDelAdd: false` (that mechanism is already an administrative replace).
